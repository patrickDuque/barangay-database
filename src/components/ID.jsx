...
export default class extends Component {
	render() {
		const getIDName = (name) => {
			const nameArray = name.split(' ');
			const filteredName = nameArray.filter((name, index) => index !== nameArray.length - 1);
					...
					<PageContainer className="uk-position-relative">
						...
						<Main className="uk-flex uk-flex-around">
							...
							<NameContainer className="uk-text-center">
								<Name className="uk-margin-remove uk-text-left">{getIDName(this.props.profile.name)}</Name>
								...
								<Paragraph className="uk-margin-remove uk-text-left">Barangay Pandayan, City of Meycauayan, Bulacan</Paragraph>
							</NameContainer>
						</Main>
					</PageContainer>
					<PageContainer className="uk-position-relative">
						...
						<div className="uk-flex uk-flex-around">
							<div>
								...
								<Paragraph className="uk-margin-remove uk-text-left">
									Contact Number: <Span>{this.props.contactNumber ? this.props.contactNumber : `(+63)___-___-____`}</Span>
								</Paragraph>
							</div>
							<div>
								<Paragraph className="uk-margin-remove">
